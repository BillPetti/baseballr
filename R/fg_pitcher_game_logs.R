...
#' @return A data frame of pitcher game logs, one row per game, with the following columns:
#'
...
fg_pitcher_game_logs <- function(playerid, year) {
  
  url <- paste0("https://www.fangraphs.com/api/players/game-log?playerid=",
                playerid,
                "&position=P&type=0&gds=&gde=&z=1703085978&season=",
                year)
  ...
  tryCatch(
    ...
    error = function(e) {
      cli::cli_alert_danger("{Sys.time()}: Invalid arguments or no pitcher game log data available!")
    },
    finally = {
    }
  )
