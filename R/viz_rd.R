...
viz_rd <- function(Tm, year) {
  ...
  if (intersect(grepl("AL|NL", Tm),
    ...
    teams <- standings_on_date_bref(paste0(year,"-06-30"), Tm) %>%        # Retrieves the teams playing in the Division or League requested (e.g. date used is 30th of June)
      as.data.frame() %>% 
      select(1) %>% 
      unlist()
    ...
    teams_al <- pbapply::pbsapply("AL Overall", standings_on_date_bref, date = paste0(year,"-06-30"))     # Retrieves the teams playing in the AL (e.g. date used is 30th of June)
    teams_nl <- pbapply::pbsapply("NL Overall", standings_on_date_bref, date = paste0(year,"-06-30"))     # Retrieves the teams playing in the NL (e.g. date used is 30th of June)
    teams <- c(teams_al, teams_nl) %>%                                        # Combines the names of both leagues teams
      as.data.frame() %>% 
      select(1, 9) %>% 
      unlist()
      rownames(teams) <- NULL
  ...
  min_RDiff <- round(min(rd$cum_R_Diff)*1.05)
  max_RDiff <- round(max(rd$cum_R_Diff)*1.05)
  ...
  map(teams_factor, function(x) {
    ...
    rd[rd$Team == x,] %>% 
      ...
      highcharter::hc_subtitle(text = paste("Through the ", year, " Season")) %>%
      highcharter::hc_credits(enabled = TRUE,                          # add credits
                              text = "Source: Baseball Reference. Using the R 'baseballr' package") %>% 
