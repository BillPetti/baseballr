...
- `ncaa_team_player_stats()` targets the current stats endpoint (#379). `stats.ncaa.org` retired the franchise-centric `/team/{team_id}/stats?game_sport_year_ctl_id=...` URL (which now returns table-less 200s) in favor of a season-team resource at `/teams/{season_team_id}/season_to_date_stats`. The function now resolves the year-specific `season_team_id` from the team's roster page and requests the new endpoint. This endpoint is gated behind Akamai's `bm-verify` interstitial challenge, which a static request cannot solve; when challenged, the function now emits an informative `cli` warning and returns an empty tibble (via the new internal `.ncaa_is_interstitial()` guard) instead of silently scraping a dead URL.
- `fg_guts()`, `fg_park()`, and `fg_park_hand()` return data again. FanGraphs replaced the legacy ASP.NET grid (HTML id `GutsBoard1_dg1_ctl00`) with a modern `.table-scroll` data grid, so the hard-coded id selector matched nothing; they now target the current grid (and use `https://`).
