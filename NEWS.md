...
- The NCAA functions (`ncaa_*()`) work again. `stats.ncaa.org` moved behind Akamai, which returns HTTP 403 "Access Denied" to requests lacking modern browser headers; `.ncaa_headers()` now sends the current client-hint / fetch-metadata headers (`sec-ch-ua`, `Sec-Fetch-*`, `Upgrade-Insecure-Requests`), and `ncaa_teams()` / `ncaa_team_player_stats()` now request over `https`. `request_with_proxy()` now sleeps 5 seconds after every request (the NCAA edge aggressively rate-limits / IP-bans) and documents passing an `httr::use_proxy()` object through `...` to route and rotate requests through a proxy. (Verified: `ncaa_roster()`, `ncaa_teams()`.)
