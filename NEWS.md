...
- NCAA scrapers (`ncaa_schedule_info()`, `ncaa_pbp()`, `ncaa_roster()`, `ncaa_team_player_stats()`, `ncaa_game_logs()`, `ncaa_lineups()`, `ncaa_teams()`) work again after `stats.ncaa.org` tightened its Akamai bot protection to return HTTP 403 to every `httr2`/`curl` request — the block is on the client's TLS/sensor fingerprint, so no header set can clear it (#410). `request_with_proxy()` now falls back, on a 403 from `stats.ncaa.org`, to a **stealth headless-Chrome** fetch (a real Chrome user-agent with the `navigator.webdriver` automation flag hidden — a vanilla headless Chrome is itself blocked by Akamai) via the optional `chromote` package, wrapping the rendered HTML in a synthetic response so every caller is unaffected. The browser session is cached and reused across calls. `chromote` + Google Chrome are an optional (`Suggests`) dependency; when absent, the scrapers emit a clear install message instead of an opaque parse error. The fast `httr2` path is still used whenever Akamai allows it, and resumes automatically if the edge relaxes.
