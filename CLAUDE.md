...
sources. It wraps the MLB Stats API, FanGraphs, Baseball Reference, Baseball
Savant / Statcast, the NCAA baseball stats site, Spotrac, the Chadwick Bureau
register, and Retrosheet, exporting ~188 functions across multiple data-source
families: `mlb_*()`, `fg_*()`, `bref_*()`, `statcast_*()` / `scrape_statcast_savant*()`,
`ncaa_*()`, `sptrc_*()`, `chadwick_*()`, `retrosheet_*()`, plus `load_*()` data
loaders, `metrics`/`woba_plus`/`fip_plus` analytics, and the `ggspraychart()`
visualization.
