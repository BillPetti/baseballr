...
(`statcast_search()` for MLB plus `statcast_search_minors()` and
`statcast_search_wbc()` for the minor-league and World Baseball Classic
routes), FanGraphs (`fg_*()`), Baseball Reference (`bref_*()`), ESPN
(`espn_mlb_*()`, `espn_college_baseball_*()`), Fox Sports
(`fox_mlb_*()`), NCAA (`ncaa_*()`), Spotrac (`sptrc_*()`), the Chadwick
Bureau register (`chadwick_*()`), and Retrosheet (`retrosheet_data()`).
...
A printable one-page reference for **`baseballr`** — the function families, the loaders, and what each one returns.

📄 **[Download the baseballr cheat sheet (PDF)](https://sportsdataverse.org/cheatsheets/baseballr.pdf)**
